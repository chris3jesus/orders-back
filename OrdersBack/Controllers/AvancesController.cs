...
namespace OrdersBack.Controllers
{
    ...
    public class AvancesController : ControllerBase
    {
        ...
        // GET: api/avances/general/id
        [HttpGet("general/{vendedor}")]
        public IActionResult GeneralVendedor(DateTime fechaInicio, DateTime fechaFin, int vendedor)
        {
            try
            {
                string periodo = fechaInicio.ToString("MMyyyy");
                ...
                var cuotasSql = $@"
                    SELECT [c].[CUO_CODVEN], COALESCE(SUM([c].[CUO_VALOR]), 0.0) AS [CUO_VALOR]
                    FROM [CUOTAVEND] AS [c]
                    WHERE [c].[CUO_CODVEN] IN ({vendedor})
                        AND [c].[CUO_PERIODO] = @periodo
                        AND [c].[CUO_CODCIA] = '01'
                        AND [c].[CUO_LINEA] < 90
                        AND [c].[CUO_LINEA] NOT IN (51, 52)
                ...
                var avancesSql = $@"
                    ...
                    FROM [FACART] AS [f]
                        ...
                        INNER JOIN [CLIENTES] AS [c] ON [c].[CLI_CODCLIE] = [f].[FAR_CODCLIE]
                            AND [c].[CLI_CODCIA] = [f].[FAR_CODCIA]
                            AND [c].[CLI_CP] = [f].[FAR_CP]
                    WHERE [c].[CLI_CODVEN] IN ({vendedor})
                        AND [f].[FAR_CODCIA] = '01'
                        AND [f].[FAR_ESTADO] != 'E'
                        AND [f].[FAR_ESTADO2] != 'L'
                        AND [f].[FAR_TIPMOV] IN (10, 97)
        ...
        // GET: api/avances/lineas/id
        [HttpGet("lineas/{vendedor}")]
        public IActionResult Lineas(DateTime fechaInicio, DateTime fechaFin, int vendedor)
        {
            try
            {
                string periodo = fechaInicio.ToString("MMyyyy");
                ...
                // Get active sellers except supervisor
                var vendedoresQuery = _context.Vemaests
                    .Where(v => v.VemCodven == vendedor && v.Supervisor != true && v.VemDesactivo != "A" && v.VemCodcia == "01")
                    .Select(v => v.VemCodven)
                    .ToList();
                ...
                // SQL for quotas
                var cuotasSql = $@"
                    SELECT [c].[CUO_LINEA] AS [CUO_CODVEN], COALESCE(SUM([c].[CUO_VALOR]), 0.0) AS [CUO_VALOR]
                    FROM [CUOTAVEND] AS [c]
                    WHERE [c].[CUO_CODVEN] = {vendedor}
                        AND [c].[CUO_PERIODO] = @periodo
                        AND [c].[CUO_CODCIA] = '01'
                        AND [c].[CUO_LINEA]  < 90
                        AND [c].[CUO_LINEA] NOT IN (51,52)
                    GROUP BY [c].[CUO_LINEA]";
                ...
                // SQL for advances
                var avancesSql = $@"
                    SELECT [a].[ART_FAMILIA] AS [FAR_CODVEN],
                        SUM([f].[FAR_CANTIDAD] / [f].[FAR_EQUIV] * [f].[FAR_PRECIO] * CASE WHEN [f].[FAR_TIPMOV] = 97 THEN -1 ELSE 1 END) AS [FAR_CANTIDAD]
                    FROM [FACART] AS [f]
                        INNER JOIN [ARTI] AS [a] ON [a].[ART_KEY] = [f].[FAR_CODART]
                            AND [a].[ART_CODCIA] = [f].[FAR_CODCIA]
                ...
                var orderedResult = result.OrderBy(r => r.Linea).ToList();
                var totalCuota = orderedResult.Sum(r => r.Cuota);
                var totalAvance = orderedResult.Sum(r => r.Avance);
                var totalSaldo = orderedResult.Sum(r => r.Saldo);
                var totalPorcentaje = totalCuota == 0 ? 0 : (totalAvance / totalCuota) * 100;
        ...
        // GET: api/avances/clientes/id
        [HttpGet("clientes/{cliente}")]
        public IActionResult Clientes(DateTime fechaInicio, DateTime fechaFin, int cliente)
        {
            try
            {
                string periodo = fechaInicio.ToString("MMyyyy");
                ...
                // Get active sellers except supervisor
                var vendedoresQuery = _context.Clientes
                    .Where(a => a.CliCodclie == cliente && a.CliCodcia == "01" && a.CliEstado == "A")
                    .Select(a => a.CliCodclie)
                    .ToList();
